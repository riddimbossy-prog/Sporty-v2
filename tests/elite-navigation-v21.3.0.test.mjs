...
assert.match(page,/elite-board-v2\.js\?v=22\.6\.0/,'Elite page must load the crest-aware renderer');
...
assert.match(page,/This week’s picks/,'summary must count this week’s published selections');
...
assert.match(boardJs,/\/api\/elite-picks\?ts=/,'renderer must read the persisted Stats2Pitch API directly');
assert.doesNotMatch(boardJs,/\blimit=10\b/,'renderer must not cap the weekly board at ten picks');
...
assert.doesNotMatch(boardJs,/Why this pick/,'public cards must not explain how a pick was produced');
assert.doesNotMatch(boardJs,/Engine score/,'public cards must not show an engine score');
